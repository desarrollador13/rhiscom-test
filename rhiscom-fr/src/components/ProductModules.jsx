...
export const ProductModule = () => {
    ...
    const loadProducts = async (pageToFetch) => {
        try {
            setLoading(true);
            const validPage = Number.isNaN(Number(pageToFetch)) ? 0 : Number(pageToFetch);
            const response = await productService.getAll(validPage, itemsPerPage);
            
            const list = Array.isArray(response) 
                ? response 
                : (response.content || response.records || response.data || []);
